...
void G_CARTESIAN::setComponent(void)
{
	...
	for (i = 0; i < cell_center.size(); i++)
	{
		...
		if (eqn_params->tracked && gas_comp(old_comp) &&
		    gas_comp(new_comp))
		{
...
void G_CARTESIAN::solveRungeKutta(int order)
{
	...
	if (st_flux == NULL)
	{
	    ...
	    /* Set coefficient a, b, c for difrerent order of RK method */
	    switch (order)
	    {
	    case 1:
		b[0] = 1.0;
	...
	double delta_t)
{
	...
	{
	case 1:
	    return addFluxInDirection1dTracked(dir,m_vst,m_flux,delta_t);
	case 2:
	    return addFluxInDirection2dTracked(dir,m_vst,m_flux,delta_t);
	    /*
	    return (eqn_params->tracked == YES) ?
		addFluxInDirection2dTracked(dir,m_vst,m_flux,delta_t) :
		addFluxInDirection2d(dir,m_vst,m_flux,delta_t);
	    */
	case 3:
	    return addFluxInDirection3dTracked(dir,m_vst,m_flux,delta_t);
	    /*
	    return (eqn_params->tracked == YES) ?
		addFluxInDirection3dTracked(dir,m_vst,m_flux,delta_t) :
		addFluxInDirection3d(dir,m_vst,m_flux,delta_t);
	    */
	}
...
void G_CARTESIAN::addFluxInDirection2d(
	...
	double delta_t)
{
	int 		i,j,n,istart,jstart,index;
	POINTER		scheme_params;
	TVD_PARAMS	TVD_params;
	EOS_PARAMS	*eos;
	int		comp;
	static SWEEP 	vst;
	static FSWEEP 	vflux;
	...
	TVD_params.lambda = delta_t/top_h[dir];
	TVD_params.beta = 0.0;
	...
	case 0:
		...
		for (i = imin[0]; i <= imax[0]; ++i)
		{
		    index = d_index2d(i,j,top_gmax);
		    comp = top_comp[index];
		    vst.dens[i+istart] = m_vst->dens[index];
		    vst.engy[i+istart] = m_vst->engy[index];
		    vst.pres[i+istart] = m_vst->pres[index];
		    vst.momn[0][i+istart] = m_vst->momn[0][index];
		    vst.momn[1][i+istart] = m_vst->momn[1][index];
		    vst.momn[2][i+istart] = 0.0;
		}	
		
		appendStencilBuffer2d(&vst,m_vst,j,dir);
		eos = &(eqn_params->eos[comp]);
		EosSetTVDParams(&TVD_params, eos);
		numericalFlux((POINTER)&TVD_params,&vst,&vflux,n);
		
		for (i = imin[0]; i <= imax[0]; ++i)
		{
		    index = d_index2d(i,j,top_gmax);
		    m_flux->dens_flux[index] += vflux.dens_flux[i+istart];
		    m_flux->engy_flux[index] += vflux.engy_flux[i+istart];
		    m_flux->momn_flux[0][index] += vflux.momn_flux[0][i+istart];
		    m_flux->momn_flux[1][index] += vflux.momn_flux[1][i+istart];
	...
	case 1:
		...
		for (j = imin[1]; j <= imax[1]; ++j)
		{
		    index = d_index2d(i,j,top_gmax);
		    comp = top_comp[index];
		    vst.dens[j+jstart] = m_vst->dens[index];
		    vst.engy[j+jstart] = m_vst->engy[index];
		    vst.pres[j+jstart] = m_vst->pres[index];
		    vst.momn[0][j+jstart] = m_vst->momn[1][index];
		    vst.momn[1][j+jstart] = m_vst->momn[0][index];
		    vst.momn[2][j+jstart] = 0.0;
		}	
		
		appendStencilBuffer2d(&vst,m_vst,i,dir);
		eos = &(eqn_params->eos[comp]);
		EosSetTVDParams(&TVD_params, eos);
		numericalFlux((POINTER)&TVD_params,&vst,&vflux,n);
		...
		{
		    index = d_index2d(i,j,top_gmax);
		    m_flux->dens_flux[index] += vflux.dens_flux[j+jstart];
		    m_flux->engy_flux[index] += vflux.engy_flux[j+jstart];
		    m_flux->momn_flux[1][index] += vflux.momn_flux[0][j+jstart];
		    m_flux->momn_flux[0][index] += vflux.momn_flux[1][j+jstart];
		    m_flux->momn_flux[2][index] = 0.0;
		}
	    }
	    break;
...
void G_CARTESIAN::addFluxInDirection1dTracked(
	int dir,
	SWEEP *m_vst,
	FSWEEP *m_flux,
	double delta_t)
{
	...
	start_clock("addFluxInDirection1dTracked");
	if (first)
	{
	    first = NO;
	    allocDirVstFlux(&vst,&vflux);
	...
	stop_clock("addFluxInDirection1dTracked");
}	/* end addFluxInDirection1dTracked */

void G_CARTESIAN::addFluxInDirection2dTracked(
	int dir,
	SWEEP *m_vst,
	FSWEEP *m_flux,
	double delta_t)
{
	...
	start_clock("addFluxInDirection2dTracked");
	if (first)
	{
	    first = NO;
	    allocDirVstFlux(&vst,&vflux);
	...
	stop_clock("addFluxInDirection2dTracked");
}	/* end addFluxInDirection2dTracked */

void G_CARTESIAN::addFluxInDirection3dTracked(
	int dir,
	SWEEP *m_vst,
	FSWEEP *m_flux,
	double delta_t)
{
	...
	case 2:
		...
		while (seg_min <= imax[2])
		{
		    ...
		    j = seg_min;
		    index = d_index3d(i,k,j,top_gmax);
		    comp = top_comp[index];
		    n = 0;
		    vst.dens[n+nrad] = m_vst->dens[index];
                    vst.engy[n+nrad] = m_vst->engy[index];
	...
	double delta_t)
{
	...
	case 0:
		...
		for (i = imin[0]; i <= imax[0]; ++i)
		{
		    ...
		    vst.momn[0][i+istart] = m_vst->momn[0][index];
		    vst.momn[1][i+istart] = m_vst->momn[1][index];
		    vst.momn[2][i+istart] = m_vst->momn[2][index];
		}	
		appendStencilBuffer3d(&vst,m_vst,j,k,dir);
		numericalFlux((POINTER)&TVD_params,&vst,&vflux,n);
		for (i = imin[0]; i <= imax[0]; ++i)
		{
		    index = d_index3d(i,j,k,top_gmax);
		    ...
		    m_flux->momn_flux[0][index] += vflux.momn_flux[0][i+istart];
		    m_flux->momn_flux[1][index] += vflux.momn_flux[1][i+istart];
		    m_flux->momn_flux[2][index] += vflux.momn_flux[2][i+istart];
		}
	    }
	    break;
	case 1:
		...
		for (j = imin[1]; j <= imax[1]; ++j)
		{
		    ...
		    vst.pres[j+jstart] = m_vst->pres[index];
		    vst.momn[0][j+jstart] = m_vst->momn[1][index];
		    vst.momn[1][j+jstart] = m_vst->momn[2][index];
		    vst.momn[2][j+jstart] = m_vst->momn[0][index];
		}
		appendStencilBuffer3d(&vst,m_vst,k,i,dir);
		numericalFlux((POINTER)&TVD_params,&vst,&vflux,n);
		for (j = imin[1]; j <= imax[1]; ++j)
		{
		    index = d_index3d(i,j,k,top_gmax);
		    ...
		    m_flux->engy_flux[index] += vflux.engy_flux[j+jstart];
		    m_flux->momn_flux[1][index] += vflux.momn_flux[0][j+jstart];
		    m_flux->momn_flux[2][index] += vflux.momn_flux[1][j+jstart];
		    m_flux->momn_flux[0][index] += vflux.momn_flux[2][j+jstart];
		}
	    }
	    break;
	case 2:
		...
		{
		    index = d_index3d(i,j,k,top_gmax);
		    vst.dens[k+kstart] = m_vst->dens[index];
		    vst.engy[k+kstart] = m_vst->engy[index];
		    vst.pres[k+kstart] = m_vst->pres[index];
		    vst.momn[0][k+kstart] = m_vst->momn[2][index];
		    vst.momn[1][k+kstart] = m_vst->momn[0][index];
		    vst.momn[2][k+kstart] = m_vst->momn[1][index];
		}	
		appendStencilBuffer3d(&vst,m_vst,i,j,dir);
		numericalFlux((POINTER)&TVD_params,&vst,&vflux,n);
		for (k = imin[2]; k <= imax[2]; ++k)
		{
		    index = d_index3d(i,j,k,top_gmax);
		    m_flux->dens_flux[index] += vflux.dens_flux[k+kstart];
		    m_flux->engy_flux[index] += vflux.engy_flux[k+kstart];
		    m_flux->momn_flux[2][index] += vflux.momn_flux[0][k+kstart];
		    m_flux->momn_flux[0][index] += vflux.momn_flux[1][k+kstart];
		    m_flux->momn_flux[1][index] += vflux.momn_flux[2][k+kstart];
		}
	    }
	    break;
	}
...
void G_CARTESIAN::augmentMovieVariables()
{
	...
	else
	{
	    ...
	    FT_ScalarMemoryAlloc((POINTER*)&hdf_movie_var,sizeof(HDF_MOVIE_VAR));
	    hdf_movie_var->num_var = num_var;
	    FT_MatrixMemoryAlloc((POINTER*)&hdf_movie_var->var_name,num_var,100,sizeof(char));
	    FT_VectorMemoryAlloc((POINTER*)&hdf_movie_var->top_var,num_var,sizeof(double*));
	...
	int dir)
{
	...
	case 0:
		...
		case DIRICHLET_BOUNDARY:
		    setDirichletStates(vst,m_vst,hs,icoords,dir,0,0,1);
		    break;
		default: 
		...
		case DIRICHLET_BOUNDARY:
		    ...
		    setDirichletStates(vst,m_vst,hs,icoords,dir,1,offset,1);
		    break;
		default: 
		    printf("ERROR appendStencilBuffer2d"
		    	   "unknown boundary type %d\n", wave_type(hs));
	...
	double chi)
{
	...
	case 1:
		...
		{
		    st.pres = min_pres;
		    m_vst->engy[index] = st.engy = EosEnergy(&st);
		}
	...
	int nb)
{
	...
	int		i,j,k,index,index0,ic[MAXD];
	GRID_DIRECTION 	ldir[3] = {WEST,SOUTH,LOWER};
	GRID_DIRECTION 	rdir[3] = {EAST,NORTH,UPPER};
	HYPER_SURF 	*hs;
	COMPONENT 	comp;
	...
	case 0:
		...
		else
		{
		    ...
		    case MOVABLE_BODY_BOUNDARY:
		    	...
		    	break;
		    case DIRICHLET_BOUNDARY:
		    	setDirichletStates(vst,m_vst,hs,ic_next,idir,nb,0,i);
		    	break;
	...
	case 1:
		...
		else
		{
		    ...
		    case MOVABLE_BODY_BOUNDARY:
		    	...
		    	break;
		    case DIRICHLET_BOUNDARY:
		    	setDirichletStates(vst,m_vst,hs,ic_next,idir,nb,
						n,i);
		    	break;
		    case FIRST_PHYSICS_WAVE_TYPE:
		    	//GFM
...
void G_CARTESIAN::get_normal_from_front()
{
	...
	double			curv,nor[3],d[3],pt1[3],w[3],f,d1,d2,d3,*pt,tol;
	int			ix, iy, iz, index;
	boolean			found;
	double			**gnor = eqn_params->gnor;
	static	int		***norset;
	...
	while (next_point(intfc,&p,&hse,&hs))
	{
	    ...
	    for (i = 0; i < dim; i++)
		if (pt[i] <= rgr->VL[i]+rgr->h[i]/2+tol || 
			pt[i] >= rgr->VU[i]-rgr->h[i]/2-tol)
		    break;
	    if (i < dim)
		continue;

	    find_block(w,ic1,pt,rgr);
	    for(i = 0; i < dim; i++)
	        pt1[i] = (ic1[i]+0.5)*rgr->h[i] + rgr->L[i];
	...
	COMPONENT	comp)
{
	...
	if (debugging("neumann_buffer"))
	{
	    ...
	    (void) printf("crx_coords = %f %f\n",crx_coords[0],crx_coords[1]);
	    (void) printf("nor = %f %f\n",nor[0],nor[1]);
	    (void) printf("vel_ref = %f %f\n",vel_ref[0],vel_ref[1]);
	}
	...
	for (i = istart; i <= nrad; ++i)
	{
	    ...
	    if (!FT_IntrpStateVarAtCoords(front,comp,coords_ref,
			m_vst->dens,getStateDens,&st_tmp.dens))
		st_tmp.dens = m_vst->dens[index];
	    if (!FT_IntrpStateVarAtCoords(front,comp,coords_ref,
			m_vst->engy,getStateEngy,&st_tmp.engy))
		st_tmp.engy = m_vst->engy[index];
	    if (!FT_IntrpStateVarAtCoords(front,comp,coords_ref,
			m_vst->pres,getStatePres,&st_tmp.pres))
		st_tmp.pres = m_vst->pres[index];
	    if (!FT_IntrpStateVarAtCoords(front,comp,coords_ref,
			m_vst->momn[0],getStateXmom,&st_tmp.momn[0]))
		st_tmp.momn[0] = m_vst->momn[0][index];
	    if (dim > 1)
		if (!FT_IntrpStateVarAtCoords(front,comp,coords_ref,
			m_vst->momn[1],getStateYmom,&st_tmp.momn[1]))
		    st_tmp.momn[1] = m_vst->momn[1][index];
	    if (dim > 2)
		if (!FT_IntrpStateVarAtCoords(front,comp,coords_ref,
			m_vst->momn[2],getStateZmom,&st_tmp.momn[2]))
		    st_tmp.momn[2] = m_vst->momn[2][index];
