        ...
        POINTER         state)
{
	...
	for (i = 0; i < dim; ++i)
	{
	    ...
	    if (!FT_IntrpStateVarAtCoords(front,comp,nsten->pts[1],
			eqn_params->vel[i],getStateVel[i],&vtmp))
            {
        	vtmp = s0->vel[i];
            }
	    s1->vel[i] = vtmp;
	...
	{
	    if (!FT_IntrpStateVarAtCoords(front,comp,nsten->pts[1],
			eqn_params->vort,getStateVort,&vort[2]))
	    	vort[2] = s0->vort;
	    s1->vort = vort[2];
	}
	if (!FT_IntrpStateVarAtCoords(front,comp,nsten->pts[1],eqn_params->pres,
                            getStatePres,&pres[2]))
	    pres[2] = s0->pres;
	if (!FT_IntrpStateVarAtCoords(front,comp,nsten->pts[1],eqn_params->dens,
                            getStateDens,&dens[2]))
	    dens[2] = s0->dens;
	s1->pres = pres[2];
	s1->dens = dens[2];
        ...
        double              *V)
{
	...
	double *m_vor = eqn_params->vort;
	double **m_vel = eqn_params->vel;
	...
	if (!FT_IntrpStateVarAtCoords(front,comp,p1,m_pres,
			getStatePres,&newst->pres))
            newst->pres = oldst->pres;
	if (!FT_IntrpStateVarAtCoords(front,comp,p1,m_dens,
			getStateDens,&newst->dens))
	    newst->dens = oldst->dens;
	if (!FT_IntrpStateVarAtCoords(front,comp,p1,m_engy,
			getStateEngy,&newst->engy))
	    newst->engy = oldst->engy;
        newst->eos = oldst->eos;
	for (i = 0; i < dim; ++i)
	{
	    newst->vel[i] = vel[i];
        ...
        double              *V)
{
	...
	POINTER sl,sr;
	COMPONENT comp;
	double pres,vort,densl,densr,engy,momn;
	...
	oldst = (STATE*)sl;
	*newst = *oldst;
	if (FT_IntrpStateVarAtCoords(front,negative_component(oldhs),p0,
				m_dens,getStateDens,&densl))
	    newst->dens = densl;
	if (FT_IntrpStateVarAtCoords(front,negative_component(oldhs),p0,
				m_engy,getStateEngy,&engy))
	    newst->engy = engy;
	for (i = 0; i < dim; ++i)
	{
	    if (FT_IntrpStateVarAtCoords(front,negative_component(oldhs),p0,
				m_mom[i],getStateMom[i],&momn))
	    	newst->momn[i] = momn;
	}
	...
	oldst = (STATE*)sr;
	*newst = *oldst;
	if (FT_IntrpStateVarAtCoords(front,positive_component(oldhs),p0,
				m_dens,getStateDens,&densr))
	    newst->dens = densr;
	if (FT_IntrpStateVarAtCoords(front,positive_component(oldhs),p0,
				m_engy,getStateEngy,&engy))
	    newst->engy = engy;
