...
#define         MAX_RING1_PTS           200
#define         MAX_RING2_PTS           1000
#define		MAX_NV			3000
#define		MAX_PTS			50
...

{
	...
	if (n >= MAX_PTS) {
	    printf("Stack overflow: n >= MAX_PTS, where n = %d.\n", n);
	    clean_up(ERROR);
	}
	...
	POINT **pts1)
{
	...
	*npts1 = 0;
	for (i = 0; i < nt; ++i)
	{
        ...
        if (*npts1 >= MAX_RING2_PTS) {
            printf("Stack overflow: *npts1 >= MAX_RING2_PTS in PointArrayRing1()\n");
            clean_up(ERROR);
        }
}	/* end PointArrayRing1 */
	...
	POINT **pts2)
{
	...
	*npts2 = 0;
	...
	if (*npts2 >= MAX_RING2_PTS) {
	    printf("Stack overflow: *npts2 >= MAX_RING2_PTS in PointArrayRing2()\n");
	    clean_up(ERROR);
	}
}	/* end PointArrayRing2 */
	...
	double		   *nor)
{
	...
	if(debugging("db_nor"))
	{
        ...
        /*
        Coords(p)[0] = old_coord_x;
        Coords(p)[1] = old_coord_y;
        Coords(p)[2] = old_coord_z;
        */
...

{
        ...
        for(i = 0; i < nt; ++i)
	{
	    ...
	    /*
            double coord1_x,coord1_y,coord1_z;
            double coord2_x,coord2_y,coord2_z;
            double coord3_x,coord3_y,coord3_z;
            double old_coord1_x,old_coord1_y,old_coord1_z;
            double old_coord2_x,old_coord2_y,old_coord2_z;
            double old_coord3_x,old_coord3_y,old_coord3_z;

            old_coord1_x = Coords(pn0)[0];
            old_coord1_y = Coords(pn0)[1];
            old_coord1_z = Coords(pn0)[2];
            old_coord2_x = Coords(pn1)[0];
            old_coord2_y = Coords(pn1)[1];
            old_coord2_z = Coords(pn1)[2];

            coord1_x = Coords(pn0)[2]*cos(Coords(pn0)[0]);
            coord1_y = Coords(pn0)[2]*sin(Coords(pn0)[0]);
            coord1_z = Coords(pn0)[1];
            coord2_x = Coords(pn1)[2]*cos(Coords(pn1)[0]);
            coord2_y = Coords(pn1)[2]*sin(Coords(pn1)[0]);
            coord2_z = Coords(pn1)[1];

            Coords(pn0)[0] = coord1_x;
            Coords(pn0)[1] = coord1_y;
            Coords(pn0)[2] = coord1_z;
            Coords(pn1)[0] = coord2_x;
            Coords(pn1)[1] = coord2_y;
            Coords(pn1)[2] = coord2_z;
	    */
	    ...
	    /*
            Coords(pn0)[0] = old_coord1_x;
            Coords(pn0)[1] = old_coord1_y;
            Coords(pn0)[2] = old_coord1_z;
            Coords(pn1)[0] = old_coord2_x;
            Coords(pn1)[1] = old_coord2_y;
            Coords(pn1)[2] = old_coord2_z;
	    */
...
{
/*
	double coordc1_theta, coordc1_z, coordc1_r;
	double coordc2_theta, coordc2_z, coordc2_r;
	double coordc3_theta, coordc3_z, coordc3_r;

	double coordr1_x, coordr1_y, coordr1_z;
	double coordr2_x, coordr2_y, coordr2_z;
	double coordr3_x, coordr3_y, coordr3_z;

	double coordc_center_theta, coordc_center_z, coordc_center_r;
	double nor_rec[3];

	coordc1_theta = Coords(Point_of_tri(tri)[0])[0];
	coordc1_z = Coords(Point_of_tri(tri)[0])[1];
	coordc1_r = Coords(Point_of_tri(tri)[0])[2];

	coordc2_theta = Coords(Point_of_tri(tri)[1])[0];
	coordc2_z = Coords(Point_of_tri(tri)[1])[1];
	coordc2_r = Coords(Point_of_tri(tri)[1])[2];

	coordc3_theta = Coords(Point_of_tri(tri)[2])[0];
	coordc3_z = Coords(Point_of_tri(tri)[2])[1];
	coordc3_r = Coords(Point_of_tri(tri)[2])[2];

	coordc_center_theta = (coordc1_theta + coordc2_theta + coordc3_theta)/3.0;
	coordc_center_r = (coordc1_r + coordc2_r + coordc3_r)/3.0;
	coordc_center_z = (coordc1_z + coordc2_z + coordc3_z)/3.0;

	coordr1_x = coordc1_r*cos(coordc1_theta);
	coordr1_y = coordc1_r*sin(coordc1_theta);
	coordr1_z = coordc1_z;

	coordr2_x = coordc2_r*cos(coordc2_theta);
	coordr2_y = coordc2_r*sin(coordc2_theta);
	coordr2_z = coordc2_z;

	coordr3_x = coordc3_r*cos(coordc3_theta);
	coordr3_y = coordc3_r*sin(coordc3_theta);
	coordr3_z = coordc3_z;

	Coords(Point_of_tri(tri)[0])[0] = coordr1_x;
	Coords(Point_of_tri(tri)[0])[1] = coordr1_y;
	Coords(Point_of_tri(tri)[0])[2] = coordr1_z;
	Coords(Point_of_tri(tri)[1])[0] = coordr2_x;
	Coords(Point_of_tri(tri)[1])[1] = coordr2_y;
	Coords(Point_of_tri(tri)[1])[2] = coordr2_z;
	Coords(Point_of_tri(tri)[2])[0] = coordr3_x;
	Coords(Point_of_tri(tri)[2])[1] = coordr3_y;
	Coords(Point_of_tri(tri)[2])[2] = coordr3_z;
*/
	double *p[3];
...
/*
	nor_rec[0] = s[2][1]*s[0][2] - s[2][2]*s[0][1];
	nor_rec[1] = s[2][2]*s[0][0] - s[2][0]*s[0][2];
	nor_rec[2] = s[2][0]*s[0][1] - s[2][1]*s[0][0];

	n[0] = -nor_rec[0]*sin(coordc_center_theta) + nor_rec[1]*cos(coordc_center_theta);
	n[1] = nor_rec[2];
	n[2] = nor_rec[0]*cos(coordc_center_theta) + nor_rec[1]*sin(coordc_center_theta);
*/
	...
	Coords(Point_of_tri(tri)[0])[2] = coordc1_r;

	Coords(Point_of_tri(tri)[1])[0] = coordc2_theta;
	Coords(Point_of_tri(tri)[1])[1] = coordc2_z;
	Coords(Point_of_tri(tri)[1])[2] = coordc2_r;

	Coords(Point_of_tri(tri)[2])[0] = coordc3_theta;
	Coords(Point_of_tri(tri)[2])[1] = coordc3_z;
	Coords(Point_of_tri(tri)[2])[2] = coordc3_r;
	*/
...
/*
	    ...
	    double nor_rec[3];
            ...
            old_coord1_x = Coords(Point_of_tri(tri)[0])[0];
            old_coord1_y = Coords(Point_of_tri(tri)[0])[1];
            old_coord1_z = Coords(Point_of_tri(tri)[0])[2];
            old_coord2_x = Coords(Point_of_tri(tri)[1])[0];
            old_coord2_y = Coords(Point_of_tri(tri)[1])[1];
            old_coord2_z = Coords(Point_of_tri(tri)[1])[2];
            old_coord3_x = Coords(Point_of_tri(tri)[2])[0];
            old_coord3_y = Coords(Point_of_tri(tri)[2])[1];
            old_coord3_z = Coords(Point_of_tri(tri)[2])[2];
	    center_x = (old_coord1_x + old_coord2_x + old_coord3_x)/3.0;
	    center_y = (old_coord1_y + old_coord2_y + old_coord3_y)/3.0;
	    center_z = (old_coord1_z + old_coord2_z + old_coord3_z)/3.0;

            coord1_x = Coords(Point_of_tri(tri)[0])[2]*cos(Coords(Point_of_tri(tri)[0])[0]);
            coord1_y = Coords(Point_of_tri(tri)[0])[2]*sin(Coords(Point_of_tri(tri)[0])[0]);
            coord1_z = Coords(Point_of_tri(tri)[0])[1];
            coord2_x = Coords(Point_of_tri(tri)[1])[2]*cos(Coords(Point_of_tri(tri)[1])[0]);
            coord2_y = Coords(Point_of_tri(tri)[1])[2]*sin(Coords(Point_of_tri(tri)[1])[0]);
            coord2_z = Coords(Point_of_tri(tri)[1])[1];
            coord3_x = Coords(Point_of_tri(tri)[2])[2]*cos(Coords(Point_of_tri(tri)[2])[0]);
            coord3_y = Coords(Point_of_tri(tri)[2])[2]*sin(Coords(Point_of_tri(tri)[2])[0]);
            coord3_z = Coords(Point_of_tri(tri)[2])[1];

            Coords(Point_of_tri(tri)[0])[0] = coord1_x;
            Coords(Point_of_tri(tri)[0])[1] = coord1_y;
            Coords(Point_of_tri(tri)[0])[2] = coord1_z;
            Coords(Point_of_tri(tri)[1])[0] = coord2_x;
            Coords(Point_of_tri(tri)[1])[1] = coord2_y;
            Coords(Point_of_tri(tri)[1])[2] = coord2_z;
            Coords(Point_of_tri(tri)[2])[0] = coord3_x;
            Coords(Point_of_tri(tri)[2])[1] = coord3_y;
            Coords(Point_of_tri(tri)[2])[2] = coord3_z;
*/
	    p[0] = Coords(Point_of_tri(tri)[0]);
	    p[1] = Coords(Point_of_tri(tri)[1]);
	    p[2] = Coords(Point_of_tri(tri)[2]);
...
/*
	    ...
	    n[0] = -nor_rec[0]*sin(center_x) + nor_rec[1]*cos(center_x);
	    n[1] = nor_rec[2];
	    n[2] = nor_rec[0]*cos(center_x) + nor_rec[1]*sin(center_x);


            Coords(Point_of_tri(tri)[0])[0] = old_coord1_x;
            Coords(Point_of_tri(tri)[0])[1] = old_coord1_y;
            Coords(Point_of_tri(tri)[0])[2] = old_coord1_z;
            Coords(Point_of_tri(tri)[1])[0] = old_coord2_x;
            Coords(Point_of_tri(tri)[1])[1] = old_coord2_y;
            Coords(Point_of_tri(tri)[1])[2] = old_coord2_z;
            Coords(Point_of_tri(tri)[2])[0] = old_coord3_x;
            Coords(Point_of_tri(tri)[2])[1] = old_coord3_y;
            Coords(Point_of_tri(tri)[2])[2] = old_coord3_z;
*/
	    return n;
	}
	else
...
LOCAL void linfit_lhf_grad_surf_point(double *us, int us_dim1, double t1[3], double t2[3], double *nrms, double *ws, double hess[3]);
	...
	int strip)
{
   	...
   	double hess[3], absnrm[3], t1_1[3], t2[3], nrm_l[3], maxprdir_l[3];
   	int t1[3];
   	double grad[2];
   	double P[3][3];
   	double H[2][2];
   	...
   	for (i=0; i<=2; i+=1)
	{
   	...
   	memset(us, 0, ((nverts - strip) << 1) * 8);
        if ((nverts - strip) < 0 || 2*(nverts - strip) > 3075) {
            printf("Stack overflow: 2*(nverts - strip) = %d.\n", 2*(nverts - strip));
            clean_up(ERROR);
        }

   	ct_set_max(bs_dim1, nverts - strip, 0);
   	memset(bs, 0, (nverts - strip) * 8);
        if ((nverts - strip) < 0 || (nverts - strip) > 1025) {
            printf("Stack overflow: (nverts - strip) = %d.\n", (nverts - strip));
            clean_up(ERROR);
        }
   	...
   	for (i=0; i<=nverts - 2; i+=1)
	{
	    ...
	    us_idx = 2 - (strip << 1) + (i << 1);
            if (us_idx < 0 || us_idx >= 3075) {
                printf("Stack overflow: us_idx = %d.\n", us_idx);
                clean_up(ERROR);
            }

      	    us[ 3 - (strip << 1) + (i << 1)] = dtemp_8;
            us_idx = 3 - (strip << 1) + (i << 1);
            if (us_idx < 0 || us_idx >= 3075) {
                printf("Stack overflow: us_idx = %d.\n", us_idx);
                clean_up(ERROR);
            }
            ...
            bs_idx = 1 - strip + i;
            if (bs_idx < 0 || bs_idx >= 1025) {
                printf("Stack overflow: bs_idx = %d.\n", bs_idx);
                clean_up(ERROR);
            }
	    /*
	      for (j=0; j<=3 - 1; j+=1)
	...
	*/
	ws_row[0] = 1.0;
	{
      	    ...
      	    for (i=1; i <= us_dim1; i+=1)
	      {
			...
			+ nrm_coor[2+i*3]*nrm_out[2];
         	...
         	ws_row[i-strip] = dtemp_10;
                if (i-strip < 0 || i-strip >= 1025) {
                    printf("Stack overflow: i-strip = %d, where us_dim1 = %d.\n", i-strip, us_dim1);
                    clean_up(ERROR);
                }
      	    }
   	}
   	...
   	*deg_out = eval_vander_bivar(us,us_dim1,bs,&bs_dim1,degree,ws_row,
   	...
   	for (i=0; i<=cs_dim1 - 1; i+=1) {
      	    cs[i] = bs[ 1 - strip_1 + i];
	    int bs_idx = 1 - strip_1 + i;
            if (bs_idx < 0 || bs_idx >= 1025) {
                printf("Stack overflow: bs_idx = %d.\n", bs_idx);
                clean_up(ERROR);
            }
            if (i >= 6) {
                printf("Stack overflow: i >= 6, where i = %d and cs_dim1 = %d.\n", i, cs_dim1);
                clean_up(ERROR);
            }
	}
   	grad[0] = cs[0];
   	...
   	for (i=0; i<=2; i+=1)
	{
	...
	else if ((us_dim1 >= 3)&&((*deg_out) > 0)) {
	  /*function linfit_lhf_grad_surf_point() */
	  for (i = 0; i < us_dim1; i++) {
	    bs1[i] = 0.0;
	    bs2[i] = 0.0;
            if (i >= MAX_NV) {
                printf("Stack overflow: i >= MAX_NV, where i = %d and us_dim1 = %d.\n", i, us_dim1);
                clean_up(ERROR);
            }
	  }
	  ...
	  for (i = 1; i < us_dim1; i++) {
	    ...
	    w = ws_row[i];
            if (i >= 1025) {
                printf("Stack overflow: i >= 1025, where i = %d and us_dim1 = %d.\n", i, us_dim1);
                clean_up(ERROR);
            }
	    if( fabs(w) > 1e-6) {
	      bs1[i] = -(nrm[0]*t1_1[0]+nrm[1]*t1_1[1]+nrm[2]*t1_1[2])/w;
	      bs2[i] = -(nrm[0]*t2[0]+nrm[1]*t2[1]+nrm[2]*t2[2])/w;
	    }
	  }
	  eval_vander_bivar(us,us_dim1,bs1,&bs_dim1,1,ws_row,
				       us_dim1, 0);
...
LOCAL void linfit_lhf_grad_surf_point(double *us, int us_dim1, double t1[3], double t2[3], double *nrms, double *ws, double hess[3])
{
  ...
  int i, bs_dim1 = us_dim1;
  int deg_out1, deg_out2;
  // static double bs1[60],bs2[60];
  static double bs1[MAX_NV],bs2[MAX_NV];
  double w;
  double nrm[3];
  ...
  for (i = 1; i < us_dim1; i++) {
    ...
    w = ws[i];
    if( fabs(w) > 1e-6) {
      bs1[i] = -(nrm[0]*t1[0]+nrm[1]*t1[1]+nrm[2]*t1[2])/w;
      bs2[i] = -(nrm[0]*t2[0]+nrm[1]*t2[1]+nrm[2]*t2[2])/w;
    }
  }
  deg_out1 = eval_vander_bivar(us,us_dim1,bs1,&bs_dim1,1,ws,
			       us_dim1, 0);
   	...
   	int strip)
{
   	...
   	static double weights[10] = {1.0,1.0,1.0,2.0,1.0,2.0,6.0,2.0,2.0,6.0};
   	double dtemp_0, dtemp_1, dtemp_2, dtemp_3, dtemp_4;
   	int i3;
	/*	FILE *file = fopen("eval_vander.txt","a"); */
   	degree_1_out = degree;
		...
		&&(degree_1_out > 1))
	{
   	...
   	memset(V, 0, npnts * ncols * 8);
        if (npnts*ncols > 28672) {
            printf("Stack overflow: npnts*ncols = %d.\n", npnts*ncols);
            clean_up(ERROR);
        }
   	...
   	for (i=0; i<=npnts - 1; i+=1)
	{
	    ...
	    V_idx = i*V_dim2;
            ...
            V_idx = i*V_dim2-1+jj;
            if (V_idx < 0 || V_idx >= 28672) {
                printf("Stack overflow: V_idx = %d.\n", V_idx);
                clean_up(ERROR);
            }
            if (2*i >= 3075) {
                printf("Stack overflow: i = %d.\n", i);
                clean_up(ERROR);
            }
   ...
   /*
   for (i = 0; i < npnts*6; i++)
     fprintf(file,"V[%d]=%.17lf\n",i,V[i]);
   for (i = 0; i < npnts; i++)
     fprintf(file,"bs[%d]=%.17lf\n",i,bs[i]);
   */
   memset(ts, 0, ncols * 8);
   if (ncols > 1024) {
       printf("Stack overflow: ncols = %d.\n", ncols);
       clean_up(ERROR);
   }
   for (i=0; i<=ncols - 1; i+=1) {
      ...
      dtemp_3 = s;
      if (fabs(dtemp_3) < 1e-6) {
         dtemp_3 = 1.0;
      }
      else {
         for (j=0; j<=V_dim1 - 1; j+=1) {
            V[ j * V_dim2 + i] =  V[ j * V_dim2 + i] / dtemp_3;
            int V_idx = j*V_dim2 + i;
            if (V_idx < 0 || V_idx >= 28672) {
                printf("Stack overflow: V_idx = %d.\n", V_idx);
                clean_up(ERROR);
            }
   ...
   for (i=0; i<=ncols - 1; i+=1) {
      ...
      for (j=1 + i; j<=ncols; j+=1) {
         ...
         for (i3=0; i3<=itemp_1 - 1; i3+=1) {
            ...
            int V_idx = (i+i3)*V_dim2 - 1 + j;
            if (V_idx < 0 || V_idx >= 28672) {
                printf("Stack overflow: V_idx = %d.\n", V_idx);
                clean_up(ERROR);
            }
            if (i3 >= 1024) {
                printf("Stack overflow: i3 = %d.\n", i3);
                clean_up(ERROR);
            }
         }
         t2 = t2 + t2;
   ...
   while ((rnk < ncols_sub)&&(degree_1_out > 1)) {
      ...
      if (degree_1_out-1 < 0 || degree_1_out-1 >= 6) {
          printf("Stack overflow: degree_1_out = %d.\n", degree_1_out);
          clean_up(ERROR);
      }
   }
   /*%%%%%%%%%%%%%%%%%%%%% */
   ...
   for (i=ncols_sub; i>=1; i+=-1) {
      ...
      for (j=1 + i; j<=ncols_sub; j+=1) {
         ...
         V_idx = (i-1)*V_dim2 - 1 + j;
         if (V_idx < 0 || V_idx >= 28672) {
             printf("Stack overflow: V_idx = %d.\n", V_idx);
             clean_up(ERROR);
         }
      }
      V_idx = (i-1)*V_dim2 - 1 + i;
      if (V_idx < 0 || V_idx >= 28672) {
          printf("Stack overflow: V_idx = %d.\n", V_idx);
          clean_up(ERROR);
      }
   ...
   for (i=0; i<=itemp_0 - 1; i+=1) {
      ...
      if (strip+i >= 10) {
          printf("Stack overflow: strip+i = %d.\n", strip+i);
          clean_up(ERROR);
      }
   }
   /*
        ...
        HYPER_SURF         *hs)
{
        ...
        if (debugging("hc_curv"))
            printf("Before set_tri_list_around_point() for pt=(%lf, %lf, %lf) in WLSP_compute_normal3d0\n", p->_coords[0], p->_coords[1], p->_coords[2]);

	tri = Tri_of_hse(p->hse);

        if (debugging("hc_curv")) {
	if (!tri)
	    printf("Tri_of_hse(p->hse) = NULL\n");
	}
	...
	/* nt1 = set_tri_list_around_point(p,tri,&ptris,intfc);*/
        ...
        if (debugging("hc_curv"))
            printf("After set_tri_list_around_point() for pt=(%lf, %lf, %lf) in WLSP_compute_normal3d0\n", p->_coords[0], p->_coords[1], p->_coords[2]);
...
EXPORT boolean WLSP_compute_normal3d_cyl(
        POINT              *p,
        HYPER_SURF_ELEMENT *hse,
        HYPER_SURF         *hs)
{
	...
	*/
        ...
        nrm_coor[0][0] = -(pc->_nor0[0])*sin( (Coords(pc)[0]) ) + (pc->_nor0[2])*cos( (Coords(pc)[0]) );
	nrm_coor[0][1] =  (pc->_nor0[0])*cos( (Coords(pc)[0]) ) + (pc->_nor0[2])*sin( (Coords(pc)[0]) );
	nrm_coor[0][2] =   pc->_nor0[1];
	...
	for(i = 0; i < np1; i++)
	    ...
	    *This part is temporarily modified for cylindrical coordinate
	    */
	    xs[i+1][0] = (Coords(pts_ring1[i])[2])*cos(Coords(pts_ring1[i])[0]);
	    xs[i+1][1] = (Coords(pts_ring1[i])[2])*sin(Coords(pts_ring1[i])[0]);
	    xs[i+1][2] = Coords(pts_ring1[i])[1];
	    ...
	    */
	    ...
	    nrm_coor[i+1][0] = -(pts_ring1[i]->_nor0[0])*sin( (Coords(pts_ring1[i]))[0] )
			       +(pts_ring1[i]->_nor0[2])*cos( (Coords(pts_ring1[i]))[0] );
	    nrm_coor[i+1][1] =  (pts_ring1[i]->_nor0[0])*cos( (Coords(pts_ring1[i]))[0] )
			       +(pts_ring1[i]->_nor0[2])*sin( (Coords(pts_ring1[i]))[0] );
	    nrm_coor[i+1][2] =   pts_ring1[i]->_nor0[1];
        }
	    ...
	    *This part is temporarily modified for cylindrical coordinate
	    */
	    xs[i+np1+1][0] = (Coords(pts_ring2[i])[2])*cos(Coords(pts_ring2[i])[0]);
	    xs[i+np1+1][1] = (Coords(pts_ring2[i])[2])*sin(Coords(pts_ring2[i])[0]);
	    xs[i+np1+1][2] = Coords(pts_ring2[i])[1];
	    ...
	    */
	    ...
	    nrm_coor[i+np1+1][1] =  (pts_ring2[i]->_nor0[0])*cos( (Coords(pts_ring2[i]))[0] )
			           +(pts_ring2[i]->_nor0[2])*sin( (Coords(pts_ring2[i]))[0] );
	    nrm_coor[i+np1+1][2] =   pts_ring2[i]->_nor0[1];
        }
	...
	*/
	pc->_nor[0] = -nrm[0]*sin(Coords(pc)[0]) + nrm[1]*cos(Coords(pc)[0]);
	pc->_nor[1] = nrm[2];
	pc->_nor[2] = nrm[0]*cos(Coords(pc)[0]) + nrm[1]*sin(Coords(pc)[0]);

        pc->curvature = -(prcurvs[0]+prcurvs[1])/2;
	return YES;
}	/* end WLSP_compute_normal3d_cyl */
 ...
 */
        ...
        double nrm[3], prcurvs[2], maxprdir[3];
	double xs[MAX_NV][3], nrm_coor[MAX_NV][3];
        ...
        if (debugging("wlsp_curvature"))
        {
            printf("Passed PointArrayRing2() for (%lf, %lf, %lf)\n", p->_coords[0], p->_coords[1], p->_coords[2]);
            printf("pc->curvature = %lf\n", pc->curvature);
        }
        ...
        if (np1 >= MAX_NV) {
            printf("Stack overflow: np1 >= MAX_NV, where np1 = %d and MAX_NV = %d\n", np1, MAX_NV);
            clean_up(ERROR);
        }
        for(i = 0; i < np1; i++)
        {
        ...
        if (np1+np2 >= MAX_NV) {
            printf("Stack overflow: np1 + np2 >= MAX_NV, where np1 + np2 = %d and MAX_NV = %d\n", np1+np2, MAX_NV);
            clean_up(ERROR);
        }
        for(i = 0; i < np2; i++)
        {
        ...
        if (debugging("wlsp_curvature"))
        {
            printf("Passed polyfit3d_lhf() for (%lf, %lf, %lf)\n", p->_coords[0], p->_coords[1], p->_coords[2]);
            printf("pc->curvature = %lf\n", pc->curvature);
        }
        ...
        if (debugging("wlsp_curvature"))
        {
            printf("Passed WLSP_compute_normal3d() for (%lf, %lf, %lf)\n", p->_coords[0], p->_coords[1], p->_coords[2]);
            printf("pc->curvature = %lf\n", pc->curvature);
        }
