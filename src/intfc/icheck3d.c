...
LOCAL   boolean tri_side_consistent_debug(TRI*,TRI_NEIGHBOR*,int,INTERFACE*);
...
EXPORT  boolean check_tri_debug(
        TRI       *tri,
        INTERFACE *intfc)
{                       
        POINT **p;      
        ...
        if (tri == NULL)                
        {                   
            (void) printf("WARNING in check_tri_debug(), tri is NULL\n");
            return NO;  
        }               
        ...
        if ((p[0] == p[1]) || (p[0] == p[2]) || (p[1] == p[2]))
        {
            (void) printf("WARNING in check_tri_debug(), two points are equal\n");
            print_tri(tri,intfc); 
            status = NO;
        }                       
        ...
        for (side = 0; side < 3; ++side)
        {               
            if (!tri_side_consistent_debug(tri,Tri_neighbor(tri)+side,side,intfc))
            {           
                (void) printf("WARNING in check_tri_debug(), "
                              "side %d of tri inconsistent\n",side);
                print_tri(tri,intfc);
                status = NO;
            }
...
LOCAL   boolean tri_side_consistent_debug(
        TRI          *tri,
