	...
	int ring)
{
	    ...
	    if (1 + np1 > MAX_RING_PTS) {
		printf("Stack overflow: 1 + np1 > MAX_RING_PTS, where np1 = %d\n.", np1);
		clean_up(ERROR);
	    }

            if (1 + np1 + np2 > MAX_RING_PTS) {
                printf("Stack overflow: 1 + np1 + np2 > MAX_RING_PTS, where np1 = %d and np2 = %d\n.", np1, np2);
                clean_up(ERROR);
            }
