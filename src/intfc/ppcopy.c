	...
	int		dst_id)
{
	...
	pp_send(TABLE_ID+tag_shf,(POINTER) intfc->table,sizeof(struct Table),dst_id);
	...
	pp_send(CHUNK_ADDR_ID+tag_shf,(POINTER)top_addr,nchunks*sizeof(POINTER),dst_id);
	free(top_addr);
	...
	int		src_id)
{
	...
	pp_recv(CHUNK_ADDR_ID+tag_shf,src_id,(POINTER)ocaddr,nchunks*sizeof(POINTER));

	//printf("id = %d  nchunks = %d\n", tag_shf, nchunks);

	for (i = 0; i < nchunks; i++)
	{
	    top = ChunkTop(new_chunk(new_table));
	    pp_recv(chunk_id(i)+tag_shf,src_id,(POINTER)top,ChunkSize(new_intfc));
