	...
	int	 **ips)
{
        ...
        for (ip[2] = smin[2]; ip[2] <= smax[2]; ++ip[2])
        {
            for (ip[1] = smin[1]; ip[1] <= smax[1]; ++ip[1])
            {
                for (ip[0] = smin[0]; ip[0] <= smax[0]; ++ip[0])
                {
                    boolean_T gb_tangle_result = 0;
                    boolean_T gf_tangle_result = 0;
                    ...
                    if (ip[0] != smax[0] && ip[1] != smax[1] && ip[2] != smax[2])
                    {
                        ...
                        int nt = T->num_of_tris[ip[2]][ip[1]][ip[0]];

			TRI **tris;

			if (nt != 0)
			{
			    tris = T->tris[ip[2]][ip[1]][ip[0]];

			    for (ii = 0; ii < nt-1; ++ii)
			    {
				for (jj = ii+1; jj < nt; ++jj)
				{
				    TRI *tri1 = tris[ii];
				    TRI *tri2 = tris[jj];

				    POINT **pstri1 = Point_of_tri(tri1);
				    POINT **pstri2 = Point_of_tri(tri2);

				    if ( (pstri1[0] == pstri2[0]) || (pstri1[0] == pstri2[1]) || (pstri1[0] == pstri2[2]) ||
					 (pstri1[1] == pstri2[0]) || (pstri1[1] == pstri2[1]) || (pstri1[1] == pstri2[2]) ||
					 (pstri1[2] == pstri2[0]) || (pstri1[2] == pstri2[1]) || (pstri1[2] == pstri2[2]) )
					continue;

				    for(kk = 0; kk < 3; kk++)
				    {
					tri1p1[kk] = (Coords(pstri1[0]))[kk];
					tri1p2[kk] = (Coords(pstri1[1]))[kk];
					tri1p3[kk] = (Coords(pstri1[2]))[kk];
					tri2p1[kk] = (Coords(pstri2[0]))[kk];
					tri2p2[kk] = (Coords(pstri2[1]))[kk];
				    ...
				    {
					gf_tangle_result = 1;
				    //triInterDetect(tri1p1, tri1p2, tri1p3, tri2p1, tri2p2, tri2p3);    //debugdan    FIXME
					break;
				    }
				}
				if (jj < nt-1)
	...
	CRX_TYPE crx_type)
{
	...
	COMPONENT 	c,cn;
        ...
        DEBUG_ENTER(remove_unphysical_crxings)
	...
	for (iy = smin[1]; iy <= smax[1]; ++iy)
	{
        ...
        DEBUG_LEAVE(remove_unphysical_crxings)
...
enum { MAX_EDGE_CRX = 200 }; /*TOLERANCE*/
        ...
        INTERFACE *intfc)
{
        ...
        free_these(5,T->seg_crx_count,T->seg_crx_lists,T->seg_crx_lists_store,
            ...
            {
                printf("\n\nStorage after free_these_T in free_crx_storage in free_grid_intfc\n");
                int totalNum = -1;
	...
	if (T->area) free_these(2,T->area,T->vol_frac);
            ...
            if (debugging("storage"))
            {
                printf("\n\nStorage after free_these_area in free_crx_storage in free_grid_intfc\n");
                int totalNum = -1;
                const char *blockName;
                blockName = "table";
                totalNum = get_number_of_blockName(stdout, blockName);
