	...
	const char	*fname)
{
	...
	if (status != GOOD_STEP)
	{
            ...
            {
                printf("\n\nStorage before free_front(*newfront) when status != GOOD_STEP in return_advance_front\n");
                int totalNum = -1;
	    ...
	    *newfront = NULL;
            ...
            if (debugging("storage"))
            {
                printf("\n\nStorage after free_front(*newfront) when status != GOOD_STEP in return_advance_front\n");
                int totalNum = -1;
                const char *blockName;
                blockName = "table";
                totalNum = get_number_of_blockName(stdout, blockName);
	...
	POINTER		wave)
{
	...
	if (front->step % Frequency_of_redistribution(front,GENERAL_WAVE) == 0 && front->dt > 1e-5)
        ...
        //if (front->step == 11922 || front->step == 11923 || front->step == 11927)
        //    do_redist = YES;
        //if (front->step == 11925)
        //    do_redist = NO;
	    ...
	    start_clock("Redistribution_Before_Global_Recon");
            ...
            */
            ...
            status = smooth_redistribute_init(front);
	    ...
	    stop_clock("Redistribution_Before_Global_Recon");
            ...
            if (debugging("storage"))
            {
                printf("\n\nStorage before *newfront = copy_front(front)\n");
                int totalNum = -1;
                const char *blockName;
                blockName = "table";
                totalNum = get_number_of_blockName(stdout, blockName);
	...
	*newfront = copy_front(front);/* f_copy_front called */
            ...
            {
		if ( (*newfront)->interf)
		    printf("(*newfront)->interf != NULL\n");
                if ( (*newfront)->grid_intfc)
                    printf("(*newfront)->grid_intfc != NULL\n");
                if ( prev_interface( (*newfront)->interf))
                    printf("(*newfront)->prev_interf != NULL\n");

                printf("\n\nStorage after *newfront = copy_front(front)\n");
                int totalNum = -1;
                const char *blockName;
                blockName = "table";
                totalNum = get_number_of_blockName(stdout, blockName);
	...
	set_copy_intfc_states(NO);
            ...
            if (debugging("storage"))
            {
                printf("\n\nStorage before (*newfront)->interf = pp_copy_interface\n");
                int totalNum = -1;
                const char *blockName;
                blockName = "table";
                totalNum = get_number_of_blockName(stdout, blockName);
	...
	(*newfront)->interf = pp_copy_interface(front->interf);
            ...
            if (debugging("storage"))
            {
                printf("\n\nStorage after (*newfront)->interf = pp_copy_interface\n");
                int totalNum = -1;
                const char *blockName;
                blockName = "table";
                totalNum = get_number_of_blockName(stdout, blockName);
    ...
    (*newfront)->interf->contactangle = front->interf->contactangle;// TODO: check contact angle value
            ...
            if (debugging("storage"))
            {
                printf("\n\nStorage before propagate_3d_front()\n");
                int totalNum = -1;
                const char *blockName;
                blockName = "table";
                totalNum = get_number_of_blockName(stdout, blockName);
    ...
    //debugdan    FIXME
            ...
            if (debugging("storage"))
            {
                printf("\n\nStorage after propagate_3d_front()\n");
                int totalNum = -1;
                const char *blockName;
                blockName = "table";
                totalNum = get_number_of_blockName(stdout, blockName);
	...
	/* char *out_name = "/nfs/scratch/hiroshima596/RT_vd_singleBubble_hp/before";
	FT_AddMovieFrame(*newfront,out_name,NO);
        //print restart/intfc files
        FT_Save(*newfront,out_name);
	int num_surfs, num_tris, num_total_tris;
	SURFACE **s;
	TRI	*t;
        for (num_surfs=num_total_tris=0, s = (*newfront)->interf->surfaces; s && *s; ++s, ++num_surfs, num_total_tris+=num_tris) {
            ...
            printf("Boundary(*s) = %d before\n", Boundary(*s));
            printf("num_tris = %d before\n", num_tris);
	}
	printf("num_surfs = %d before\n", num_surfs);
        printf("num_total_tris = %d before\n", num_total_tris); */


            if (debugging("hp_storage"))
            {
                printf("\n\nStorage before Hiprop\n");
                int totalNum = -1;
                const char *blockName;
                blockName = "table";
                totalNum = get_number_of_blockName(stdout, blockName);
	...
	// TODO: normal and curvature are not printed correctly by 2nd FT_AddMovieFrame()
        printf("\n+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
        printf("Entering Hiprop library\n");
	fflush(stdout);
/*
        hiPropMesh* hpmesh1;
        hpInitMesh(&hpmesh1);
        printf("After hpInitMesh\n");
        fflush(stdout);

        ImportMeshToHiprop((*newfront)->interf, hpmesh1, (*newfront)->pp_grid, 0);
        printf("After import, num_points = %d\n", hpmesh1->ps->size[0]);
        fflush(stdout);
        ExportMeshToFronTier(hpmesh1, *newfront, 2, 3);
        hpDeleteMesh(&hpmesh1);

        scatter_front(*newfront);
*/
        printf("Leaving Hiprop library\n");
        printf("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n\n");


            if (debugging("hp_storage"))
            {
                printf("\n\nStorage after Hiprop\n");
                int totalNum = -1;
                const char *blockName;
                blockName = "table";
                totalNum = get_number_of_blockName(stdout, blockName);
...
/*
        ...
        for (num_surfs=num_total_tris=0, s = (*newfront)->interf->surfaces; s && *s; ++s, ++num_surfs, num_total_tris+=num_tris) {
            ...
            printf("num_tris = %d after\n", num_tris);
        }
        printf("num_surfs = %d after\n", num_surfs);
        printf("num_total_tris = %d after\n", num_total_tris); */
		...
		/* Redistribute the New Front */
            ...
            if (debugging("storage"))
            {
                printf("\n\nStorage after smoothing algo Hiprop() and before redistribute(*newfront)\n");
                int totalNum = -1;
                const char *blockName;
                blockName = "table";
                totalNum = get_number_of_blockName(stdout, blockName);
	...
	if (debugging("final_front"))
	    print_Front_structure(front);
            ...
            if (debugging("storage"))
            {
                printf("\n\nStorage after redistribute(*newfront) and reset_normal_on_intfc\n");
                int totalNum = -1;
                const char *blockName;
                blockName = "table";
                totalNum = get_number_of_blockName(stdout, blockName);
        ...
        if (debugging("tri_consistency") && !consistent_interface((*newfront)->interf))
        {
            screen("ERROR after redistribute() for proc #%d ts=%d, "
                   "(*newfront)->interf is inconsistent\n", pp_mynode(),(*newfront)->step);
            clean_up(ERROR);
        }

        int nt, nf, nl, ns;
        nt = nf = nl = ns = 0;
        INTERFACE *intfc = (*newfront)->interf;
	SURFACE **s;
        RECT_GRID *gr = (*newfront)->rect_grid;
	TRI *tri;
	static boolean first_badAngle = true, first_large = true;
        for (s = intfc->surfaces; s && *s ; ++s) {
            for (tri=first_tri(*s); !at_end_of_tri_list(tri,*s); tri=tri->next) {
                ...
                case BAD_ANGLE:
                    ...
                    if(debugging("bad_tri_consistency") && (*newfront)->step == 905 &&
                       (pp_mynode() == 2186 || pp_mynode() == 2042)) {
			if (!check_tri_debug(tri, intfc))
			    clean_up(ERROR);
		    }


	            if(debugging("prt_bad_tri") && (*newfront)->step == 905 &&
		       (pp_mynode() == 2186 || pp_mynode() == 2042))
		    {
            		char  ch[60];
			FILE *db_file;

            		sprintf(ch, "badAngle_tris_ts%d_proc%d.plt", (*newfront)->step, pp_mynode());
            		db_file = fopen(ch, "a");
			if (first_badAngle) {
            		    fprintf(db_file, "TITLE = \"tecplot surface\"\n"
                        	    "VARIABLES = \"x\", \"y\", \"z\"\n");
		            fprintf(db_file, "ZONE T=\"%s\" N=%d E=%d\nF=FEPOINT, ET=TRIANGLE\n",
                             		"badAngle_tris", 3*100, 100);
			    first_badAngle = false;
			}
                	fprintf(db_file,"%-9g %-9g %-9g\n",p0[0],p0[1],p0[2]);
                        fprintf(db_file,"%-9g %-9g %-9g\n",p1[0],p1[1],p1[2]);
                        fprintf(db_file,"%-9g %-9g %-9g\n",p2[0],p2[1],p2[2]);
        	    	fclose(db_file);


                        sprintf(ch, "badAngle_nbrs_ts%d_proc%d_tri%d.plt", (*newfront)->step, pp_mynode(), nf);
                        db_file = fopen(ch, "a");
                        fprintf(db_file, "TITLE = \"tecplot surface\"\n"
                                "VARIABLES = \"x\", \"y\", \"z\"\n");
                        fprintf(db_file, "ZONE T=\"%s\" N=%d E=%d\nF=FEPOINT, ET=TRIANGLE\n",
				...
				printf("is_side_bdry(tri%d, side%d) = YES.\n", nf, i);
				fprintf(db_file,"%-9g %-9g %-9g\n",p0[0],p0[1],p0[2]);
                                fprintf(db_file,"%-9g %-9g %-9g\n",p1[0],p1[1],p1[2]);
                                fprintf(db_file,"%-9g %-9g %-9g\n",p2[0],p2[1],p2[2]);
				continue;
                                ...
                                TRI  *tri_nb = (tri->neighbor[i]).tri;
                                for (j=0; j<3; ++j) {
                                    fprintf(db_file,"%-9g %-9g %-9g\n", Coords(Point_of_tri(tri_nb)[j])[0],
                                            Coords(Point_of_tri(tri_nb)[j])[1], Coords(Point_of_tri(tri_nb)[j])[2]);
                                }
			    }
                        }
                        for(i=0; i<4; i++)
                            fprintf(db_file, "%d %d %d\n", 3*i+1, 3*i+2, 3*i+3);
                        fclose(db_file);
		    }
                ...
                case LARGE:
                       ...
                       (pp_mynode() == 2186 || pp_mynode() == 2042))
                    {
                        ...
                        sprintf(ch, "large_tris_ts%d_proc%d.plt", (*newfront)->step, pp_mynode());
                        db_file = fopen(ch, "a");
			if (first_large) {
                            fprintf(db_file, "TITLE = \"tecplot surface\"\n"
                                    "VARIABLES = \"x\", \"y\", \"z\"\n");
                            fprintf(db_file, "ZONE T=\"%s\" N=%d E=%d\nF=FEPOINT, ET=TRIANGLE\n",
                                        "large_tris", 3*100, 100);
			    first_large = false;
			}
                        fprintf(db_file,"%-9g %-9g %-9g\n",p0[0],p0[1],p0[2]);
                        fprintf(db_file,"%-9g %-9g %-9g\n",p1[0],p1[1],p1[2]);
                        ...
                        sprintf(ch, "large_nbrs_ts%d_proc%d_tri%d.plt", (*newfront)->step, pp_mynode(), nl);
                        db_file = fopen(ch, "a");
                        fprintf(db_file, "TITLE = \"tecplot surface\"\n"
                                "VARIABLES = \"x\", \"y\", \"z\"\n");
                        fprintf(db_file, "ZONE T=\"%s\" N=%d E=%d\nF=FEPOINT, ET=TRIANGLE\n",
	...
	boolean		reconstruct_front)
{
	...
	if (front->_point_propagate != NULL)
	{
	    ...
	    start_clock("normal_propagate");
            ...
            if (debugging("storage"))
            {
                printf("\n\nStorage before make_tri_comp_lists() in propagate_3d_front()\n");
                int totalNum = -1;
                const char *blockName;
                blockName = "table";
                totalNum = get_number_of_blockName(stdout, blockName);
	    ...
	    tri_list_status =
            ...
            if (debugging("storage"))
            {
                printf("\n\nStorage after make_tri_comp_lists() in propagate_3d_front()\n");
                int totalNum = -1;
                const char *blockName;
                blockName = "table";
                totalNum = get_number_of_blockName(stdout, blockName);
	    ...
	    /*set_wall_flag_for_surface(front->interf); */
            ...
            if (debugging("storage"))
            {
                printf("\n\nStorage before propagate_surface_points in propagate_3d_front()\n");
                int totalNum = -1;
                const char *blockName;
                blockName = "table";
                totalNum = get_number_of_blockName(stdout, blockName);
    ...
    //debugdan    FIXME
            ...
            if (debugging("storage"))
            {
                printf("\n\nStorage after propagate_surface_points in propagate_3d_front()\n");
                int totalNum = -1;
                const char *blockName;
                blockName = "table";
                totalNum = get_number_of_blockName(stdout, blockName);
	...
	if (reconstruct_front == YES)
	{
        ...
        //if (front->step == 9305 || front->step == 9460 || front->step == 9605 || front->step == 9680 || front->step == 9825 || front->step == 9940 || front->step == 10020 || front->step == 10065 || front->step == 10125 || front->step == 10410 || front->step == 10445 || front->step == 10755 || front->step == 11530 || front->step == 11565 || front->step == 11570 || front->step == 11620 || front->step == 11720 || front->step == 11730 || front->step == 11740 || front->step == 11755 || front->step == 11760 || front->step == 11780 || front->step == 11785 || front->step == 11790 || front->step == 11805 || front->step == 11810 || front->step == 11865 || front->step == 11875 || front->step == 11880 || front->step == 11890)
        //    set_cut_ref(NO);
	...
	start_clock("scatter_front");
            ...
            if (debugging("storage"))
            {
                printf("\n\nStorage before scatter_front() in propagate_3d_front()\n");
                int totalNum = -1;
                const char *blockName;
                blockName = "table";
                totalNum = get_number_of_blockName(stdout, blockName);
	...
	if (!scatter_front(newfront))
	{
	    ...
	    *dt_frac *= TIME_STEP_REDUCTION_FACTOR(front->interf);
	    DEBUG_LEAVE(propagate_3d_front)
            ...
            if (debugging("storage"))
            {
                printf("\n\nStorage after scatter_front() in propagate_3d_front()\n");
                int totalNum = -1;
                const char *blockName;
                blockName = "table";
                totalNum = get_number_of_blockName(stdout, blockName);
...
//        (void) next_point(intfc,NULL,NULL,NULL);
//        while (next_point(intfc,&p,&hse,&hs))
//	    printf("p->curvature = %lf in init_intfc_curvature3d()\n", p->curvature);

	if (debugging("hc_curv"))
        printf("\nPassed mean_curvature_at_point() in init_intfc_curvature3d \n");
