	...
	INIT_DATA *init)
{
	...
	minimum_angle_at_triangle_vertex(init,VECTOR_WAVE) = radians(15.0);
        //maximum_scaled_triangle_side_length(init) = 1.0; //bring error "tri cannot match" for RS_RV
	...
	F_BASIC_DATA *ft_basic)
{
	...
	else 	/* use supplied data to initialize front */
	{
		...
		set_size_of_intfc_state(front->sizest);/* set global variable size_of_intfc_state */
        	...
        	f_set_interface_hooks(dim,init_data(&Init));/* most function pointers in INTERFACE set here */
		...
		if (ft_basic->RestartRun)
		{
		    ...
		    comp_grid.dim = dim;
                    ...
                    for (i = 0; i < ft_basic->dim; ++i)
                        front->gmax_new_pp_grid[i] = ft_basic->gmax_new_pp_grid[i];
...
/*
                    {
                        printf("\n\nStorage before read_print_interface() in FT_StartUp()\n");
                        int totalNum = -1;
...
*/
		...
		else
		{
		     ...
		     *Now the front->rect_grid is the global grid */
        	    ...
        	    i_intfc(&Init) = front->interf = make_interface(dim);/* Init intfc here */

        	    copy_rect_grid(computational_grid(front->interf),
                            front->rect_grid);

        	    set_default_front(&Init,front);/*most function pointers in Front set here */
                    set_default_comp(NO);
		    delete_interface(front->interf);
		    set_size_of_intfc_state(front->sizest);

		    /* Why do we have to make interface twice? */
        	    i_intfc(&Init) = front->interf = make_interface(dim);
	...
	LEVEL_FUNC_PACK *level_func_pack)
{
	...
	const double eps = 10.0*MACH_EPS;
        ...
        pp_clip_rect_grids(front,rbt);
        //printf("debugdan - after pp_clip_rect_grids, front->rect_grid->gmax: %d %d %d.\n",
	...
	if (level_func_pack->set_3d_bdry)
	{
	    clip_front_to_subdomain(front);
            ...
            {
                screen("ERROR after clip_front_to_subdomain(), "
