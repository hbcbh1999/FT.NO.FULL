	    ...
	    for (iy = iy1; iy <= iy2; ++iy)
	    {
			...
			if(debugging("line_tri"))
			    ...
			    *psurf = *s;
			    ...
			    if (num >= MAX_CURVE_CRX_SORT) {
				printf("Stack overflow: num >= MAX_CURVE_CRX_SORT!\n");
				clean_up(ERROR);
			    }
			}
		    }
	...
	int		*smax)
{
	...
	if (nsurf > MAX_3_COMP_CURV) {
	    printf("Stack overflow: nsurf > MAX_3_COMP_CURV, where nsurf = %d.\n", nsurf);
	    clean_up(ERROR);
	}
	...
	for (iz = smin[2]; iz <= smax[2]; ++iz)
	{
	    ...
	    for (iy = smin[1]; iy <= smax[1]; ++iy)
	    {
		    ...
		    if( (comp[d] != NO_COMP) || 
		        ...
		        continue;
		    comp[d] = cc;
                    intfc->default_comp = cc;  /* Bug fix for the wrong phase problem of the default_comp */
		}
	    }
	}
