	...
	Front		*right)
{
	...
	set_size_of_intfc_state(size_of_state(right->interf));
	set_copy_intfc_states(YES);
	left->interf = pp_copy_interface(right->interf);
	(void) delete_interface(right->interf);
        ...
        Front              *fr)
{
        ...
        double    tmp[3],x[5],nor[3],nor_cyl[3],local_x[3],local_y[3];
	...
	/*
	if(fr->coordinate == 'c')
	    normal(p,hse,hs,nor_cyl,fr);
	else if(fr->coordinate == 'r')
	    normal(p,hse,hs,nor,fr);
	*/
	if(fr->coordinate == 'c')
	    sine_weighted_normal3d(p,hse,hs,nor_cyl);
	else if(fr->coordinate == 'r')
	...
	if(fr->coordinate == 'c')
	{
	    ...
	    old_coord1_x = Coords(pn)[0];
	    old_coord1_y = Coords(pn)[1];
	    old_coord1_z = Coords(pn)[2];

	    coord1_x = Coords(pn)[2]*cos(Coords(pn)[0]);
	    coord1_y = Coords(pn)[2]*sin(Coords(pn)[0]);
	    coord1_z = Coords(pn)[1];

	    Coords(pn)[0] = coord1_x;
	    Coords(pn)[1] = coord1_y;
	    Coords(pn)[2] = coord1_z;
	}
	...
	if(fr->coordinate == 'c')
	{
	    Coords(pn)[0] = old_coord1_x;
	    Coords(pn)[1] = old_coord1_y;
	    Coords(pn)[2] = old_coord1_z;

	    nor[0] = -nor_cyl[0]*sin(old_coord_x) + nor_cyl[2]*cos(old_coord_x);
	    nor[1] = nor_cyl[0]*cos(old_coord_x) + nor_cyl[2]*sin(old_coord_x);
	    nor[2] = nor_cyl[1];
	}
	    ...
	    if (fr->coordinate == 'c')
	    {
        	old_coord2_x = Coords(p_list[n])[0];
        	old_coord2_y = Coords(p_list[n])[1];
        	old_coord2_z = Coords(p_list[n])[2];
	
        	coord2_x = Coords(p_list[n])[2]*cos(Coords(p_list[n])[0]);
        	coord2_y = Coords(p_list[n])[2]*sin(Coords(p_list[n])[0]);
        	coord2_z = Coords(p_list[n])[1];
	
        	Coords(p_list[n])[0] = coord2_x;
        	Coords(p_list[n])[1] = coord2_y;
        	Coords(p_list[n])[2] = coord2_z;
	    }
	    ...
	    if (fr->coordinate == 'c')
	    {
		Coords(p_list[n])[0] = old_coord2_x;
		Coords(p_list[n])[1] = old_coord2_y;
		Coords(p_list[n])[2] = old_coord2_z;
	    }
