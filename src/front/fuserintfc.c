        ...
        Front           *front)
{
        ...
        if (debugging("wlsp_curvature"))
            printf("Entering f_wlsp_curvature() for pt=(%lf, %lf, %lf)\n", p->_coords[0], p->_coords[1], p->_coords[2]);

	INTERFACE *intfc = hs->interface;


        if (debugging("wlsp_curvature"))
            printf("intfc->normal_unset = %d\n", intfc->normal_unset);
	...
	CURVE *c;
        ...
        if (debugging("wlsp_curvature"))
	printf("Passed f_WLSP_set_intfc_geom() in f_wlsp_curvature()\n");
	...
	INTERFACE *intfc)	/* intfc may not be the same as front->interf */
{
        ...
        if (debugging("wlsp_curvature"))
            printf("Entering WLSP_set_intfc_geom()\n");
		...
		while (next_point(intfc,&p,&hse,&hs))
        	...
        	printf("Passed WLSP_compute_normal3d0() for pt=(%lf, %lf, %lf)\n", p->_coords[0], p->_coords[1], p->_coords[2]);
		...
		while (next_point(intfc,&p,&hse,&hs))
		{
			...
			p->curvature = f_mean_curvature_at_point3d(p,hse,hs,front);
                    ...
                    if (isnan(p->curvature)) {
                        ...
                        if (debugging("wlsp_curvature"))
                        printf("Passed isnan(p->curvature), p->curvature = %lf\n", p->curvature);
                    }
                        ...
                        if (debugging("wlsp_curvature"))
                        printf("Passed fabs(p->curvature) > max_curvature, p->curvature = %lf\n", p->curvature);
                    }
