	...
	double t)
{
    ...
    {
        if ( (ppz == 0 && k == kmin && dir == LOWER) || (ppz == 0 && k == kmin-1 && dir == UPPER) )
        {
        ...
        else if ( (ppz == ppgmax[2]-1 && k == kmax && dir == UPPER) || (ppz == ppgmax[2]-1 && k == kmax+1 && dir == LOWER) )
        {
...
void Incompress_Solver_Smooth_3D_Cylindrical::setInitialCondition()
{
	...
	// Initialize state at cell_center
	    ...
	    if (getInitialState != NULL)
            ...
            cell_center[i].m_state.m_U[0] = 0.3985;
...
void Incompress_Solver_Smooth_3D_Cylindrical::computeSubgridModel(void)
{
        ...
        for (i = 0; i <= ((imax-imin+1)/NB)-1; i++)
        {
            ...
            index000 = d_index3d(ii,jj,kk,top_gmax);
                ...
                ma11 = (2.0*delta2*(sum_ss11/(NB^3)))
                        - (2.0*tdelta2*(sum_s/(NB^3))*(sum_s11/(NB^3)));
                ma12 = (2.0*delta2*(sum_ss12/(NB^3)))
                        - (2.0*tdelta2*(sum_s/(NB^3))*(sum_s12/(NB^3)));
                ma13 = (2.0*delta2*(sum_ss13/(NB^3)))
                        - (2.0*tdelta2*(sum_s/(NB^3))*(sum_s13/(NB^3)));
                ma22 = (2.0*delta2*(sum_ss22/(NB^3)))
                        - (2.0*tdelta2*(sum_s/(NB^3))*(sum_s22/(NB^3)));
                ma23 = (2.0*delta2*(sum_ss23/(NB^3)))
                        - (2.0*tdelta2*(sum_s/(NB^3))*(sum_s23/(NB^3)));
                ma33 = (2.0*delta2*(sum_ss33/(NB^3)))
                        - (2.0*tdelta2*(sum_s/(NB^3))*(sum_s33/(NB^3)));
                la11 = (sum_vel_uu/(NB^3))-((sum_vel_u/(NB^3))*(sum_vel_u/(NB^3)));
                la12 = (sum_vel_uv/(NB^3))-((sum_vel_u/(NB^3))*(sum_vel_v/(NB^3)));
                la13 = (sum_vel_uw/(NB^3))-((sum_vel_u/(NB^3))*(sum_vel_w/(NB^3)));
        ...
        for (i = 0; i <= ((imax-imin+1)/NB)-1; i++)
        {
            ...
            for(iiii = ii; iiii < ii+NB; iiii++)
            {
                ...
                cell_center[index].m_state.m_mu += cs*delta2*s[index];
            }
        }
