	...
	INIT_PHYSICS	*ip)
{
	...
	if (strcmp(s,"RT") == 0 || strcmp(s,"rt") == 0)
	    ...
	    init_type = RT_AMB;
	else if(strcmp(s,"RV") == 0 || strcmp(s,"rv") == 0)
		init_type = RT_VEL;				
...
/* For multiple bubble description, make Fourier analysis of the front  */
	...
	switch (bubble_type)
	{
	case MULTI_MODE_FILE:

            screen("Enter the name of the file containing the\n"
                   "initial interface perturbation data in the\n"
                   "x - direction\n"
            ...
            screen("Enter the name of the file containing the\n"
                   "initial velocity amplitude data.\n"
                    "\tThe full directory path name of the file "
                    "is preferred: ");
        ...
        for (iii = 0; iii < 3; ++iii)
			...
			while(flag == 0)
            {
                ...
                else
                {
                    ...
                    {
                        rsp_a->mode[count/2].wave_number[0] = value;
                        rsp_b->mode[count/2].wave_number[0] = value;
                    }
                    else //This is an amplitude
                    {
                        rsp_a->mode[count/2].amplitude = value;
                        rsp_b->mode[count/2].amplitude = value;

                        //value = PI*drand48();
                        //if(drand48() > 0.5)
                        //    value = -1*value;

                        //value = random_gaussian(av_phase,P_sd,xsubi_p);
                        value = PI*(2.0*erand48(xsubi_p) - 1.0);

                        rsp_a->mode[count/2].phase = value;//log10(rsp_a->mode[count/2].wave_number[0])/log(2);
                        rsp_b->mode[count/2].phase = value;//log10(rsp_a->mode[count/2].wave_number[0])/log(2);
                        rsp_a->mode[count/2].vel_phase = value;//log10(rsp_a->mode[count/2].wave_number[0])/log(2); 
                        rsp_b->mode[count/2].vel_phase = value;//log(rsp_a->mode[count/2].wave_number[0])/log(2);
                    }
                    count++;
                }
            }
            ...
            while(flag == 1)
            {
                ...
                else
                {
                    ...
                    if(count%2 == 0) //This is a wave number value
                    {
                        rsp_a->mode[count/2].wave_number[1] = value;
                        rsp_b->mode[count/2].wave_number[1] = value;
                    }
                    else //This is an amplitude
                    {
                        rsp_a->mode[count/2].amplitude = value;
	       	        ...
	       	        rsp_a->mode[count/2].phase = value;//log(rsp_a->mode[count/2].wave_number[1])/log(2);
                        rsp_b->mode[count/2].phase = value;//log(rsp_a->mode[count/2].wave_number[1])/log(2);
                    }
                    count++;
                }
            }
            ...
            for(i = 0; i < count/2; i++)
                screen("\nindex = %d,k_x = %2.10lf,"
                       ...
                       rsp_a->mode[i].wave_number[0],
                       rsp_a->mode[i].wave_number[1],
                       rsp_a->mode[i].amplitude,rsp_a->mode[i].phase);

            screen("\n");
/*
			rsp_a->mode[0].phase = rsp_a->mode[0].vel_phase = 0.75*rsp_a->mode[0].temp_phase+0.25*rsp_a->mode[1].temp_phase;
			rsp_b->mode[0].phase = rsp_b->mode[0].vel_phase = 0.75*rsp_b->mode[0].temp_phase+0.25*rsp_b->mode[1].temp_phase;
			
			for(i = 1; i < count/2 - 1; i++)
			{
				rsp_a->mode[i].phase = rsp_a->mode[i].vel_phase = 0.5*(0.5*rsp_a->mode[i-1].temp_phase+rsp_a->mode[i].temp_phase+0.5*rsp_a->mode[i+1].temp_phase);
				rsp_b->mode[i].phase = rsp_b->mode[i].vel_phase = 0.5*(0.5*rsp_b->mode[i-1].temp_phase+rsp_b->mode[i].temp_phase+0.5*rsp_b->mode[i+1].temp_phase);
			}
			

			rsp_a->mode[count/2-1].phase = rsp_a->mode[count/2-1].vel_phase = 0.75*rsp_a->mode[count/2-1].temp_phase+0.25*rsp_a->mode[count/2-2].temp_phase;
			rsp_b->mode[count/2-1].phase = rsp_b->mode[count/2-1].vel_phase = 0.75*rsp_b->mode[count/2-1].temp_phase+0.25*rsp_b->mode[count/2-2].temp_phase;

			for(i = 0; i < count/2; i++)
				screen("%2.10lf\t%lf\n",rsp_a->mode[i].wave_number[0], rsp_a->mode[i].phase);
*/
            flag = 0;
            save_count = count;
            count = 0;
            while(flag == 0)
            {
                ...
                else
                {
                    ...
                    fgets(s, 12, vdata);
                    value = atof(s);
	       	    if(count%2 != 0) //This is an amplitude value
                    {
                        rsp_a->mode[count/2].vel_amplitude = value;
                        rsp_b->mode[count/2].vel_amplitude = value;
                        ...
                        //rsp_a->mode[count/2].vel_phase = value;
                        //rsp_b->mode[count/2].vel_phase = value;
                    }
                    else
                    {
                        rsp_a->mode[count/2].wave_number[0] = value;
            ...
            //Zero out velocity amplitudes for remaining wavenumbers.
/*
            for(i = count; i < save_count; i++)
            {
                if(i%2 != 0)
                {
                    rsp_a->mode[i/2].vel_amplitude = 0.0;
                    rsp_b->mode[i/2].vel_amplitude = 0.0;
                    rsp_a->mode[i/2].vel_phase = 0.0;
                    rsp_b->mode[i/2].vel_phase = 0.0;
                }
            }
*/
            //Print wavenumbers and their amplitudes.
            for(i = 0; i < count/2; i++)
                screen("\nindex = %d, k_x = %2.10f,"
                       "v_amp = %2.10f, v_phase = %2.10f\n", i, rsp_a->mode[i].wave_number[0],
                       rsp_a->mode[i].vel_amplitude, rsp_a->mode[i].vel_phase);
...
/*
        C(rsp_a->amb_st) = 0.0;
        CI(rsp_a->amb_st) = 0.0;
        SCT(rsp_a->amb_st) = 0.0;
        SCT0(rsp_a->amb_st) = 0.0;
        PRT(rsp_a->amb_st) = 0.0;
        STR(rsp_a->amb_st) = 0.0;
        DCSn(rsp_a->amb_st) = 0.0;
        DCIn(rsp_a->amb_st) = 0.0;
        CSn(rsp_a->amb_st) = 0.0;
        CIn(rsp_a->amb_st) = 0.0;
        CSt1(rsp_a->amb_st) = 0.0;
        DCSt1(rsp_a->amb_st) = 0.0;
        Svisx(rsp_a->amb_st) = 0.0;
        Svisy(rsp_a->amb_st) = 0.0;
        Svisz(rsp_a->amb_st) = 0.0;
        Sethermal(rsp_a->amb_st) = 0.0;
*/
#endif /* defined SUBGRID */
        Dvnnn(rsp_a->amb_st) = 0.0;
        Dvnn(rsp_a->amb_st) = 0.0;
        Dvt1nn(rsp_a->amb_st) = 0.0;
        Dvt2nn(rsp_a->amb_st) = 0.0;
        Dvt1n(rsp_a->amb_st) = 0.0;
        Dvt2n(rsp_a->amb_st) = 0.0;
        Mvisx(rsp_a->amb_st) = 0.0;
        Mvisy(rsp_a->amb_st) = 0.0;
        Mvisz(rsp_a->amb_st) = 0.0;
        Evis(rsp_a->amb_st) = 0.0;
        Ethermal(rsp_a->amb_st) = 0.0;
        Cmd(rsp_a->amb_st)[0] = 0.0;
        Cmd(rsp_a->amb_st)[1] = 0.0;
        Emd(rsp_a->amb_st) = 0.0;
        Dvt1t1(rsp_a->amb_st) = 0.0;
        Dvt2t1(rsp_a->amb_st) = 0.0;
...
/*
        C(rsp_b->amb_st) = 0.0;
        CI(rsp_b->amb_st) = 0.0;
        SCT(rsp_b->amb_st) = 0.0;
        SCT0(rsp_b->amb_st) = 0.0;
        PRT(rsp_b->amb_st) = 0.0;
        STR(rsp_b->amb_st) = 0.0;
        DCSn(rsp_b->amb_st) = 0.0;
        DCIn(rsp_b->amb_st) = 0.0;
        CSn(rsp_b->amb_st) = 0.0;
        CIn(rsp_b->amb_st) = 0.0;
        CSt1(rsp_b->amb_st) = 0.0;
        DCSt1(rsp_b->amb_st) = 0.0;
        Svisx(rsp_b->amb_st) = 0.0;
        Svisy(rsp_b->amb_st) = 0.0;
        Svisz(rsp_b->amb_st) = 0.0;
        Sethermal(rsp_b->amb_st) = 0.0;
*/
#endif /* defined SUBGRID */
        Dvnnn(rsp_b->amb_st) = 0.0;
        Dvnn(rsp_b->amb_st) = 0.0;
        Dvt1nn(rsp_b->amb_st) = 0.0;
        Dvt2nn(rsp_b->amb_st) = 0.0;
        Dvt1n(rsp_b->amb_st) = 0.0;
        Dvt2n(rsp_b->amb_st) = 0.0;
        Mvisx(rsp_b->amb_st) = 0.0;
        Mvisy(rsp_b->amb_st) = 0.0;
        Mvisz(rsp_b->amb_st) = 0.0;
        Evis(rsp_b->amb_st) = 0.0;
        Ethermal(rsp_b->amb_st) = 0.0;
        Cmd(rsp_b->amb_st)[0] = 0.0;
        Cmd(rsp_b->amb_st)[1] = 0.0;
        Emd(rsp_b->amb_st) = 0.0;
        Dvt1t1(rsp_b->amb_st) = 0.0;
        Dvt2t1(rsp_b->amb_st) = 0.0;
	...
	int		stype)
{
	...
	Locstate	amb_st;
	int		i, m, dim, number_modes, sign;
	float		g_z,z_intfc_pert;
	static Locstate tmp_st = NULL;
	float		crds[MAXD];
	float  vel_temp, ratio;
	...
	isothermal_stratified_state(s,coords[dim-1]-RS->z_intfc,g_z,amb_st);
        ...
        // 0.3cm   (0.087)  hc->0.15  sf->0.0087
/*
        fprintf(stdout, "init_mass_diffusion 0.15\n");
	if(comp == 3)
	{
		pdens(s)[0] = Dens(s)-(1.0-fabs(erf((z_intfc_pert-coords[dim-1])/0.15)));
		pdens(s)[1] = 0.0+0.5*(1.0-fabs(erf((z_intfc_pert-coords[dim-1])/0.15)));
		Dens(s) = pdens(s)[0]+pdens(s)[1];
	}
	else
	{
	     pdens(s)[0] = 0.0+(1.0-fabs(erf((z_intfc_pert-coords[dim-1])/0.15)));
             pdens(s)[1] = Dens(s)-0.5*(1.0-fabs(erf((z_intfc_pert-coords[dim-1])/0.15)));
             Dens(s) = pdens(s)[0]+pdens(s)[1];
	}
*/

        fprintf(stdout, "init_mass_diffusion 0.005\n");
        if(comp == 3)
        {
                pdens(s)[0] = Dens(s)-(1.0-fabs(erf((z_intfc_pert-coords[dim-1])/0.005)));
                pdens(s)[1] = 0.0+0.5*(1.0-fabs(erf((z_intfc_pert-coords[dim-1])/0.005)));
                Dens(s) = pdens(s)[0]+pdens(s)[1];
        }
        else
        {
             pdens(s)[0] = 0.0+(1.0-fabs(erf((z_intfc_pert-coords[dim-1])/0.005)));
             pdens(s)[1] = Dens(s)-0.5*(1.0-fabs(erf((z_intfc_pert-coords[dim-1])/0.005)));
             Dens(s) = pdens(s)[0]+pdens(s)[1];
        }
 
  		//Set new density values for initial mass diffusion layer.
		//screen("x = %f, y = %f, z = %f\n", coords[0], coords[1], coords[2]);

                //if(coords[0] > 14.2 && coords[0] <= 14.3 && coords[1] > 9.3 && coords[1] < 9.4)
                //screen("z = %f Dens(s) = %f\n", coords[2], Dens(s));
	...
	if(comp == 3)
        {
                pdens(s)[0] = Dens(s)-0.945*(1.0-fabs(erf((z_intfc_pert-coords[dim-1])/0.087)));
                pdens(s)[1] = 0.0+0.5*(1.0-fabs(erf((z_intfc_pert-coords[dim-1])/0.087)));
                Dens(s) = pdens(s)[0]+pdens(s)[1];
        }
        else
        {
                pdens(s)[0] = 0.0+0.945*(1.0-fabs(erf((z_intfc_pert-coords[dim-1])/0.087)));
                pdens(s)[1] = Dens(s)-0.5*(1.0-fabs(erf((z_intfc_pert-coords[dim-1])/0.087)));
                Dens(s) = pdens(s)[0]+pdens(s)[1];
        }
        */
        ...
        if(comp == 3)
        {
                pdens(s)[0] = Dens(s)-0.945*(1.0-fabs(erf((z_intfc_pert-coords[dim-1])/0.011)));
                pdens(s)[1] = 0.0+0.5*(1.0-fabs(erf((z_intfc_pert-coords[dim-1])/0.011)));
                Dens(s) = pdens(s)[0]+pdens(s)[1];
        }
        else
        {
                pdens(s)[0] = 0.0+0.945*(1.0-fabs(erf((z_intfc_pert-coords[dim-1])/0.011)));
                pdens(s)[1] = Dens(s)-0.5*(1.0-fabs(erf((z_intfc_pert-coords[dim-1])/0.011)));
                Dens(s) = pdens(s)[0]+pdens(s)[1];
        }
        */
	...
	if (RS->init_type == RT)
	{
 	...
 	else if(RS->init_type == RT_VEL)
        {
        /*
        for(m = 0; m < number_modes; ++m)
        {
	        if(coords[dim-1] <= z_intfc_pert)
                    sign = -1;
                else
                    sign = 1;
                Vel(s)[dim-1] += -1*sign*RS->mode[m].vel_amplitude*exp(-1*(RS->mode[m].wave_number[0])*fabs(z_intfc_pert-coords[dim-1]))*sin(RS->mode[m].wave_number[0]*coords[0] - RS->mode[m].vel_phase);
	}
        */

        //for(m = 0; m < 22; ++m)  //no_long
        for(m = 0; m < 25; ++m)
        {
                if(coords[dim-1] <= z_intfc_pert)
                    sign = -1;
                ...
                Vel(s)[dim-1] += -1*sign*RS->mode[m].vel_amplitude*exp(-1*(RS->mode[m].wave_number[0])*fabs(z_intfc_pert-coords[dim-1]))*sin(RS->mode[m].wave_number[0]*coords[0] - RS->mode[m].vel_phase);
                //fprintf(stdout, "before vel init INFO Vel(s)[dim-1] %e m %d number_modes %d coords[dim-1] %e sign %d vel_amplitude %e wave_number[0] %e vel_phase %e coords[0] %e, after  %e %e %e\n",Vel(s)[dim-1],m,number_modes,coords[dim-1],sign,RS->mode[m].vel_amplitude,RS->mode[m].wave_number[0],RS->mode[m].vel_phase,coords[0],cos(RS->mode[m].wave_number[0]*coords[0] - RS->mode[m].vel_phase),exp(-1*(RS->mode[m].wave_number[0])*fabs(z_intfc_pert-coords[dim-1])),RS->mode[m].vel_amplitude);
	}
        ...
        */
        ...
        Vel(s)[dim-1] = Vel(s)[dim-1]*0.2108;
	//Vel(s)[dim-1] = sign*fabs(Vel(s)[dim-1])*0.2108;
        //Vel(s)[dim-1] = Vel(s)[dim-1]*0.0002108;
        //if(Vel(s)[dim-1] != 0.0)

        //if(Vel(s)[dim-1] < 0.0 && (coords[dim-1] > z_intfc_pert))
        //    fprintf(stdout, "NEGATIVE VEL %e\n", Vel(s)[dim-1]);
        //fprintf(stdout, "with vel init %e %e %e\n",Vel(s)[0],Vel(s)[1],Vel(s)[2]);
	//screen("vel = %f\n", Vel(s)[dim-1]);
	set_state(s,stype,s);
