...
void Incompress_Solver_Smooth_3D_Cartesian::compDiffWithSmoothProperty_velocity_MAC_coupled_vd(void)
{
        ...
        for (i = imin; i <= imax; i++)
        {
            ...
            if (bNoBoundary[2] == 3) // SOUTH FACE is on REFLECT BC
            {
                if (SLIP)
                {
                    ...
                    //rhs += coeff0_old[2]*(U0_nb[2] - U0_center); // This expression is O.K.
                    // U0_nb[2] = U0_center
                    rhs += 0.0;
                }
                else
                {
                    // onWallvel = 0
                    // NO SLIP CONDITION
                    solver.Add_A(I*3,I*3, 2.0*coeff0[2]);
            ...
            if (bNoBoundary[3] == 3) // NORTH FACE is on REFLECT BC
            {
                ...
                else
                {
                    // onWallvel = 0
                    // NO SLIP CONDITION
                    solver.Add_A(I*3,I*3, 2.0*coeff0[3]);
                    rhs += coeff0_old[3]*(U0_nb[3] - U0_center);
                }
            }
            ...
            if (bNoBoundary[0] == 3) // WEST FACE is on REFLECT BC
            {
                ...
                else
                {
                    // NO SLIP CONDITION
                    // onWallVel = 0.0
                    solver.Add_A(I*3+1,I*3+1, 2.0*coeff1[0]);
                    rhs += coeff1_old[0]*(U1_nb[0] - U1_center);
                }
            }
            ...
            if (bNoBoundary[1] == 3) // EAST FACE is on REFLECT BC
            {
                ...
                else
                {
                    // NO SLIP CONDITION
                    // onWallVel = 0.0
                    solver.Add_A(I*3+1,I*3+1, 2.0*coeff1[1]);
                    rhs += coeff1_old[1]*(U1_nb[1] - U1_center);
                }
            }
            ...
            {
                printf("NO MORE BOUNDARY CONDITION IMPLEMENTED at line %d in function %s\n", __LINE__, __func__);
                assert(false);
            }
            ...
            else if (bNoBoundary[3] == 3)// NORTH FACE is on BC
            {
                ...
                {
                    // u[0]
                    solver.Add_A(I*3+1,I_nb[0]*3, -temp2);
                    coeff_temp_old  = 1.0/2*m_dt/rho*mu_old[0]/(top_h[0]*top_h[1]);
                    coeff_temp_old -= 1.0/3*m_dt/rho*mu_old[2]/(top_h[0]*top_h[1]);
                    rhs += coeff_temp_old*U0_nb[0];
            ...
            //consider LOWER, UPPER, NORTH
            if (!bNoBoundary[4] && !bNoBoundary[5] && !bNoBoundary[3]) // PERIODIC or INTERIOR
            {
                //w[3]
            ...
            else if (bNoBoundary[3] == 3) // NORTH FACE
            {
                //hardwired for slip condition
            ...
            */
            ...
            if (bNoBoundary[0] == 3) // WEST FACE is on REFLECT BC
            {
                ...
                {
                    // NO SLIP CONDITION
                    solver.Add_A(I*3+2,I*3+2, 2.0*coeff2[0]);
                    rhs += coeff2_old[0]*(U2_nb[0] - U2_center);
                }
            }
            else if (!bNoBoundary[0]) // PERIODIC or INTERIOR
            {
            ...
            if (bNoBoundary[1] == 3) // EAST FACE is on REFLECT BC
            {
                ...
                {
                    // NO SLIP CONDITION
                    solver.Add_A(I*3+2,I*3+2, 2.0*coeff2[1]);
                    rhs += coeff2_old[1]*(U2_nb[1] - U2_center);
                }
            }
            else if (!bNoBoundary[1]) // PERIODIC or INTERIOR
            {
            ...
            if (bNoBoundary[2] == 3) // SOUTH FACE is on REFLECT BC
            {
                ...
                {
                    solver.Add_A(I*3+2,I*3+2, 2.0*coeff2[2]);
                    rhs += coeff2_old[1]*(U2_nb[2] - U2_center);
                }
            }
            else if (!bNoBoundary[2]) // PERIODIC or INTERIOR
            {
                solver.Add_A(I*3+2,I*3+2, coeff2[2]);
            ...
            if (bNoBoundary[3] == 3) // NORTH FACE is on REFLECT BC
            {
                ...
                {
                    // NO SLIP CONDITION
                    solver.Add_A(I*3+2,I*3+2, 2.0*coeff2[3]);
                    rhs += coeff2_old[3]*(U2_nb[3] - U2_center);
                }
            }
            else if (!bNoBoundary[3]) // PERIODIC or INTERIOR
            {
            ...
            if (!bNoBoundary[4] && !bNoBoundary[5]) // INTERIOR
            {
                ...
                solver.Add_A(I*3+2,I_nb[4]*3+2, -coeff2[4]);
        ...
        if (debugging("step_size"))
	{
         ...
         * removal tag: HAOZ
         * */
        enforceVecState(vecarray); // on m_U_tmp
        for (l = 0; l < 3; l++)
        for (k = 0; k <= top_gmax[2]; k++)
        for (j = 0; j <= top_gmax[1]; j++)
        for (i = 0; i <= top_gmax[0]; i++)
...
void Incompress_Solver_Smooth_3D_Cartesian::compAdvectionTerm_MAC_decoupled_vd(int flag, boolean bGhostCell)
{
    ...
    if (!flag)
    {
        ...
        // removal tag: HAOZ
        // REFLECTION B.C.
        enforceVecState(vecarray);//m_U_center_bar
        for (l = 0; l < 3; l++)
        for (k = 0; k <= top_gmax[2]; k++)
        for (j = 0; j <= top_gmax[1]; j++)
        for (i = 0; i <= top_gmax[0]; i++)
        ...
        enforceVecState(vel);//m_U_face_bar
        for (l = 0; l < 3; l++)
        for (k = 0; k <= top_gmax[2]; k++)
        for (j = 0; j <= top_gmax[1]; j++)
        for (i = 0; i <= top_gmax[0]; i++)
        ...
        //removal tag: HAOZ
        enforceVecState(vel); // on m_U_face_bar
        for (l = 0; l < dim; ++l)
        for (k = 0; k <= top_gmax[2]; k++)
        for (j = 0; j <= top_gmax[1]; j++)
        for (i = 0; i <= top_gmax[0]; i++)
        ...
        if (debugging("step_size"))
        {
             ...
             * */
            enforceVecState(vel);// on m_adv terms
            for (l = 0; l < 3; l++)
            for (k = 0; k <= top_gmax[2]; k++)
            for (j = 0; j <= top_gmax[1]; j++)
            for (i = 0; i <= top_gmax[0]; i++)
        ...
        int *icoords)
{
    ...
    */
    ...
    // NEUMANN BOUNDARY CONDITION FOR W velocity on Z Direction
    if (bNoBoundary[4]==2 && xyz==2) //for w on LOWER boundary cells
        tmp = 0.5*(cell_center[index].m_state.m_U[xyz] + 0.0);
    else if (bNoBoundary[5]==2 && xyz==2) //for w on UPPER boundary cells
...
void Incompress_Solver_Smooth_3D_Cartesian::printInteriorVelocity(char *out_name)
{
	    ...
	    pointsz = top_gmax[2] + 1;
            ...
            if(iFparams->movie_option->plot_velo)
            {
                ...
                fprintf(outfile, "VECTORS grad_phi double\n");
                for(i = 0; i <= top_gmax[0]; ++i)
                for(j = 0; j <= top_gmax[1]; ++j)
                for(k = 0; k <= top_gmax[2]; ++k)
                {
                    index = d_index3d(i,j,k,top_gmax);
                    vel_x = cell_center[index].m_state.grad_q[0];
                    vel_y = cell_center[index].m_state.grad_q[1];
...
void Incompress_Solver_Smooth_3D_Cartesian::computeGradientQ_MAC_vd(void)
{
    ...
    enforceVecState(vecarray);// on grad_q
    for (l = 0; l < dim; l++)
    for (k = 0; k <= top_gmax[2]; k++)
    for (j = 0; j <= top_gmax[1]; j++)
    for (i = 0; i <= top_gmax[0]; i++)
...
// NO SLIP CONDITION
void Incompress_Solver_Smooth_3D_Cartesian::NeumannBC(double **solute)
{
    int i, j, k;
...
void Incompress_Solver_Smooth_3D_Cartesian::ReflectBC(double **solute)
{
    int i, j, k;
    int InIndex, BIndex;
    //printf("in func %s lbuf = [%d %d %d] ubuf = [%d %d %d]\n", __func__,lbuf[0], lbuf[1], lbuf[2], ubuf[0],ubuf[1], ubuf[2]);
    ...
    if (machine_index[0] == GP[0][0] && rect_boundary_type(intfc,0,0) == REFLECTION_BOUNDARY)
    {
     ...
     for (j = 0; j <= top_gmax[1]; j++)
     {
         ...
         BIndex = d_index3d(imin-1,j,k,top_gmax);
         solute[0][BIndex] = 0.0; // On wall normal velocity component vanish
    ...
    if (machine_index[0] == GP[0][1] && rect_boundary_type(intfc,0,1) == REFLECTION_BOUNDARY)
    {
     ...
     for (j = 0; j <= top_gmax[1]; j++)
     {
         ...
         BIndex = d_index3d(imax,j,k,top_gmax);
         solute[0][BIndex] = 0.0; // On wall normal velocity component vanish
         //tangential velocities
         for (i = imax-ubuf[0]+1; i <= imax; i++)// X Upper
         {
             InIndex = d_index3d(i,j,k,top_gmax);//This is Interior State index
             BIndex = d_index3d(2*imax+1-i,j,k,top_gmax);//This is Boundary State index
             if (SLIP)
             {
                 solute[1][BIndex] = solute[1][InIndex];
                 solute[2][BIndex] = solute[2][InIndex];
    ...
    if (machine_index[1] == GP[1][0] && rect_boundary_type(intfc,1,0) == REFLECTION_BOUNDARY)
    {
     ...
     for (i = 0; i <= top_gmax[0]; i++)
     {
         ...
         BIndex = d_index3d(i,jmin-1,k,top_gmax);
         solute[1][BIndex] = 0.0; // On wall normal velocity component vanish
         //tangential velocities
    ...
    if (machine_index[1] == GP[1][1] && rect_boundary_type(intfc,1,1) == REFLECTION_BOUNDARY)
    {
     ...
     for (i = 0; i <= top_gmax[0]; i++)
     {
         ...
         BIndex = d_index3d(i,jmax,k,top_gmax);
         solute[1][BIndex] = 0.0; // On wall normal velocity component vanish
         //tangential velocities
         for (j = jmax-ubuf[1]+1; j <= jmax; j++)//Y Upper
         {
             InIndex = d_index3d(i,j,k,top_gmax);//This is Interior State index
             BIndex = d_index3d(i,2*jmax+1-j,k,top_gmax);//This is Boundary State index
             if (SLIP)
             {
                  solute[0][BIndex] = solute[0][InIndex];
                  solute[2][BIndex] = solute[2][InIndex];
...
//Reflection Boundary Condition Treatment for Vector
//function name enforceReflectionState() => enforceVecState()
void Incompress_Solver_Smooth_3D_Cartesian::enforceVecState(double **vel)
{
     int       dir, side;
     INTERFACE *intfc = front->interf;
